...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let args = Cli::parse();
    let ctrl = ContainerAppController::new(QuadletManager::default());
...
fn run_cli(cmd: &Command, ctrl: &ContainerAppController<QuadletManager>) {
    match cmd {
        ...
        Command::Start { name } => match ctrl.start(name) {
        ...
        Command::Stop { name } => match ctrl.stop(name) {
        ...
        Command::Logs { name, lines } => {
            for line in ctrl.logs(name, *lines) {
                println!("{line}");
            }
        }
    }
